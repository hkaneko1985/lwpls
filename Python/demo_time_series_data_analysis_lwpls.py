...
if dynamics_max:
    ...
    for x_variable_number in range(dataset.shape[1] - 1):
        dataset_with_dynamics = np.append(dataset_with_dynamics, dataset[dynamics_max:, x_variable_number+1:x_variable_number+2], axis=1)
        for time_delay_number in range(int(np.floor(dynamics_max / dynamics_span))):
            dataset_with_dynamics = np.append(dataset_with_dynamics, dataset[dynamics_max-(time_delay_number+1)*dynamics_span:-(time_delay_number+1)*dynamics_span, x_variable_number+1:x_variable_number+2], axis=1)
else:
...
for test_sample_number in range(len(y_test_with_999)):
    ...
    if test_sample_number - y_measurement_delay >= 0:
        if y_test_with_999[test_sample_number-y_measurement_delay] != 999:
            x_train = np.append(x_train, x_test_with_999[test_sample_number-y_measurement_delay:test_sample_number-y_measurement_delay+1,], axis=0)
